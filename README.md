...
-->
- **Add words without leaving Obsidian.** Select a word (or phrase) and a **＋ Add to Inoh** button pops up right above it; on mobile the same action lives in the long-press menu. There's also an **Add selected word to deck** command, which works on the word under the cursor too. Inflected forms find their entry (*flipped* adds *flip*), and if the word has several meanings you pick the right one from a list. A word Inoh doesn't have yet can be saved to your drafts and finished in the Inoh app, where you generate your own card for it or request it for the public dictionary.
